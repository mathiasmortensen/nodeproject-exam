...
<svelte:head><title>UpLoL | Champion {champ.name}</title></svelte:head>

<main class="min-h-screen mt-7 bg-zinc-950/80 px-4 py-8 text-zinc-100">
    ...
    <section class="mt-6 border border-zinc-800 bg-zinc-900 p-6">
      {#if champ && champ.image}
        ...
        <div class="mb-8 border-b border-zinc-800 pb-6">
          ...
          <div class="border border-zinc-800 bg-zinc-950 p-4 flex items-start gap-4">
            <img
          ...
          {#each champ.spells as spell, i}
            <div class="border border-zinc-800 bg-zinc-950 p-4 mt-4 flex items-start gap-4">
              <img
        ...
        <div>
          ...
          <div class="grid grid-cols-4 gap-4 mb-8 border-b border-zinc-800 pb-6">
            {#each champ.skins.filter((skin) => !('parentSkin' in skin)) as skin, i}
              {#if !skinError[i]}
                <div class="overflow-hidden border border-zinc-800 bg-zinc-950">
                  <img
        ...
        <div class="mt-8 text-center">
          <p class="text-zinc-100">
            For more information on {champ.name}
          </p>
          <a
